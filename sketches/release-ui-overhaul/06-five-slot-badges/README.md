...
- The trash icon remains geometrically centered in whatever red width is currently exposed: its compositor offset is calculated as half the reveal width minus the icon's right-padding/half-width baseline. This holds for partial, settled, and full-row swipes.
- Release has two deliberate outcomes: drags under 60px reset; partial qualifying drags settle to an 88px trash action that still opens confirmation when tapped. A complete swipe reaching at least 92% of the measured row width commits deletion automatically after the 200ms full-row dismissal. A cancelled touch can never auto-delete.
