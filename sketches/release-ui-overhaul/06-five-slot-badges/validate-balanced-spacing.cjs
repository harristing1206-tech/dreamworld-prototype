...
assert.match(html, /<header class="alarm-head"><div class="list-toolbar"><button class="list-edit" id="editAlarms"[^>]*>Edit<\/button><button[^>]*aria-label="Add alarm"><svg[\s\S]*?<path d="M12 5v14M5 12h14"\/><\/svg><\/button><\/div><h1 class="tab-heading">Good Morning, Harris<\/h1><\/header>/, 'Alarm header does not preserve personalized primary heading with Edit and Add controls');
