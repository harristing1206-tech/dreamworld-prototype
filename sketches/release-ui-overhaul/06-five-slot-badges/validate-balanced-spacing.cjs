...
assert.match(html, /<header class="alarm-head"><div class="list-toolbar"><button class="list-edit header-action" id="editAlarms"[^>]*>Edit<\/button><button class="alarm-add header-action" id="addAlarm"[^>]*aria-label="Add alarm"[\s\S]*?<\/button><\/div><h1 class="nav-title">Alarm<\/h1><\/header>/, 'Alarm action rail does not place Edit left and Add right');
