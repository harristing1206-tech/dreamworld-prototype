...
(async () => {
  ...
  assert.deepEqual(tabs.map(tab => tab.getAttribute('aria-label')), ['Alarm', 'History', 'Capture a dream', 'Insights', 'Profile']);
  assert.deepEqual(tabs.map(tab => tab.querySelector('.tab-label')?.textContent.trim()), ['Alarm', 'History', undefined, 'Insights', 'Profile'], 'bottom navigation must label four destinations while the center Capture action stays icon-only');
  assert.ok(d.querySelector('[data-tab="log"] .plus-disc .capture-tab-mic'), 'center Capture action must be a raised microphone disc');
  ...
  assert.equal(d.querySelector('.latest-dream-label').textContent,'Latest dream');
  assert.equal(d.getElementById('latestDreamRailDate').textContent,'Aug 25');
  assert.equal(d.getElementById('latestDreamRailDate').dateTime,'2026-08-25');
  assert.equal(d.getElementById('latestDreamRailTitle').textContent,'The station beneath the lake');
  assert.equal(d.getElementById('latestDreamRail').getAttribute('aria-label'),'Open latest dream, The station beneath the lake');
  d.getElementById('latestDreamRail').click();
  assert.ok(d.querySelector('[data-screen="history"]').classList.contains('active'));
  assert.equal(d.getElementById('historyFocusTitle').textContent,'The station beneath the lake');
  ...
  d.getElementById('addAlarm').focus();d.getElementById('addAlarm').click();
  ...
  d.getElementById('saveAlarm').click();await wait(0);
  assert.equal(d.querySelectorAll('.alarm-row').length, before + 1);
  assert.equal(d.querySelectorAll('.alarm-row img').length, 0);
  assert.equal(d.activeElement,d.getElementById('addAlarm'),'closing the Add Alarm sheet must restore focus to its contextual trigger');
  ...
  assert.equal(d.getElementById('addAlarm').hidden,true,'Add Alarm must be suppressed during list edit mode');
  d.getElementById('editAlarms').click();
  assert.equal(d.getElementById('addAlarm').hidden,false,'Add Alarm must return when list edit mode ends');
