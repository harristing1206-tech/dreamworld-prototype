...
(async () => {
  ...
  assert.equal(d.getElementById('alarmCreateRow').hidden,true,'Add Alarm must be suppressed during list edit mode');
  d.getElementById('editAlarms').click();
  assert.equal(d.getElementById('alarmCreateRow').hidden,false,'Add Alarm must return when list edit mode ends');
