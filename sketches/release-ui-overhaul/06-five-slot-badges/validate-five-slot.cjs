...
(async () => {
  ...
  const captureReadyGradientRules=source.match(/[^{}]*\.capture-redesign \.capture-ready\{[^}]*\}/g)||[];
  assert.equal(captureReadyGradientRules.length,2,'only light and dark Capture ready rules may add the supplied ambient gradient');
  assert.ok(captureReadyGradientRules.every(rule=>/radial-gradient/i.test(rule)),'Capture ready state must own its reference gradient');
  const withoutApprovedGradients=source.replace(/[^{}]*\.history-reference-remodel \.history-focus-hero\{[^}]*\}/g,'').replace(/[^{}]*\.capture-redesign \.capture-ready\{[^}]*\}/g,'');
  assert.doesNotMatch(withoutApprovedGradients,/(?:linear|radial)-gradient/i,'gradients must remain scoped to the supplied History hero and Capture ready state');
  ...
  assert.match(source, /#alarmList \.alarm-row\.next-alarm\{/, 'NEXT WAKE styling must follow the computed next enabled alarm');
  assert.match(source, /\.alarm-list \.alarm-row:not\(\.next-alarm\)\{/, 'secondary alarm styling must exclude the computed next alarm');
  assert.doesNotMatch(source, /#alarmList \.alarm-row:first-child/, 'DOM position must not determine NEXT WAKE styling');
  ...
  assert.equal(d.getElementById('insightAverageDream').textContent, '1:14');
  ...
  assert.equal(d.querySelector('.latest-dream-label').textContent,'Latest log');
  ...
  assert.match(d.getElementById('latestDreamRailExcerpt').textContent,/clear water/);
  ...
  assert.equal(d.querySelector('.profile-head h1').textContent, 'Profile');
  assert.equal(d.querySelector('.profile-identity-copy strong').textContent, 'Harris');
  assert.equal(d.querySelector('.profile-identity-copy span').textContent, 'Private profile');
  ...
  assert.equal(d.getElementById('pauseRecordingLabel').textContent.trim(),'Resume','paused recording must expose a visible Resume label');
  assert.equal(d.getElementById('recordingLiveLabel').textContent.trim(),'Paused','paused recording must not claim to be live');
  ...
  pauseRecording.click();
  assert.equal(pauseRecording.getAttribute('aria-label'),'Pause recording');
  assert.equal(d.getElementById('pauseRecordingLabel').textContent.trim(),'Pause','resumed recording must restore the visible Pause label');
  assert.equal(d.getElementById('recordingLiveLabel').textContent.trim(),'Live','resumed recording must restore the live state');
  ...
  const historyCountBeforeFailedCommit=d.querySelectorAll('#historyList .history-entry').length;
  dom.window.__failDraftDelete=true;
  d.getElementById('saveDream').click();
  await wait(30);
  assert.ok(d.querySelector('[data-log-state="transcript"]').classList.contains('active'),'failed atomic journal commit must keep the transcript retryable');
  assert.equal(d.querySelectorAll('#historyList .history-entry').length,historyCountBeforeFailedCommit,'failed draft deletion must not add a journal row');
  assert.equal(dom.window.__idbStores.get('journal').size,0,'journal store must remain unchanged after rollback');
  assert.ok(dom.window.__idbStores.get('drafts').has('latest'),'raw draft must remain after rollback');
  dom.window.__failDraftDelete=false;
  d.getElementById('saveDream').click();
  await wait(30);
  ...
  d.querySelector('[data-tab="log"]').click();
  await wait(10);
  assert.ok(d.querySelector('[data-log-state="ready"]').classList.contains('active'),'returning to Capture after a successful save must start a fresh capture');
  assert.equal(d.querySelectorAll('#historyList .history-entry').length,1,'revisiting Capture must not duplicate the saved recording');
  ...
  const editedAlarmID=firstAlarm.dataset.id;
  ...
  const updatedAlarm=d.querySelector(`#alarmList .alarm-row[data-id="${editedAlarmID}"]`);
  assert.equal(updatedAlarm.querySelector('.alarm-value').textContent,'9:15');
  assert.equal(updatedAlarm.querySelector('.alarm-period').textContent,'PM');
  assert.match(updatedAlarm.querySelector('.alarm-detail').textContent,/Early class/);
