...
assert.match(html,/\.setting-icon\{width:24px[^}]*background:transparent/,'Settings icon tiles remain');assert.match(html,/\.profile-avatar\{width:64px;height:64px/,'Profile identity avatar geometry is inconsistent');assert.match(html,/\.profile-identity\{[^}]*min-height:92px/,'Profile identity row hierarchy missing');
