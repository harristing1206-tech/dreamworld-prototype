...
test('sample edits persist across reload and save restores focus by record identity',async()=>{
 const first=makeDom();await wait(first.dom);const d=first.dom.window.document;d.querySelector('[data-tab="history"]').click();const opener=d.querySelector('.history-entry[data-entry-id="sample-2026-08-25"]');opener.focus();opener.click();d.getElementById('historyFocusEdit').click();await wait(first.dom);
 d.getElementById('dreamEditName').value='Persistent edit check';d.getElementById('saveDreamEdit').click();await wait(first.dom);
 assert.equal(d.activeElement.id,'historyFocusEdit','focus must return to the detail Edit action');
 assert.equal(d.querySelector('.history-entry[data-entry-id="sample-2026-08-25"] h2').textContent,'Persistent edit check');
 assert.equal(d.getElementById('historyFocusTitle').textContent,'Persistent edit check');
...
test('confirmed sample deletion persists across reload',async()=>{
 const first=makeDom();await wait(first.dom);const d=first.dom.window.document;d.querySelector('[data-tab="history"]').click();d.getElementById('editDreams').click();d.querySelector('.history-entry[data-entry-id="sample-2026-08-25"]').click();d.getElementById('deleteSelectedDreams').click();d.getElementById('confirmDelete').click();await wait(first.dom,30);
...
test('History selection exposes pressed cards while legacy delete affordances stay unreachable',async()=>{
 ...
 d.getElementById('editDreams').click();const first=d.querySelector('#historyList .history-entry');
 assert.ok(minuses().every(button=>button.tabIndex===-1&&button.getAttribute('aria-hidden')==='true'));assert.ok(deletes().every(button=>button.tabIndex===-1&&button.getAttribute('aria-hidden')==='true'));assert.equal(first.getAttribute('aria-pressed'),'false');first.click();assert.equal(first.getAttribute('aria-pressed'),'true');assert.equal(d.getElementById('historySelectionActions').hidden,false);
