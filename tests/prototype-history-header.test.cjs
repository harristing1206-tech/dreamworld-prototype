...
test('History uses one clear title while Edit anchors top-left', () => {
  ...
  assert.match(html, /<header class="home-header">[\s\S]*?<button class="list-edit header-action" id="editDreams"[^>]*>Edit<\/button><span aria-hidden="true"><\/span>[\s\S]*?<h1 class="greeting">Dream journal\.<\/h1>[\s\S]*?<\/header>/);
