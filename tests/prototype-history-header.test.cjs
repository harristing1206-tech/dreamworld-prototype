...
test('History uses one clear title while Edit anchors top-right', () => {
  ...
  assert.match(html, /<header class="home-header">[\s\S]*?<span aria-hidden="true"><\/span><button class="list-edit" id="editDreams"[^>]*>Edit<\/button>[\s\S]*?<h1 class="greeting">Dream journal\.<\/h1>[\s\S]*?<\/header>/);
