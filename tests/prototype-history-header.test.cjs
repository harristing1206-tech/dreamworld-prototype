...
test('History compact orientation leads from the top-left while Edit anchors top-right', () => {
  ...
  assert.match(html, /<header class="home-header">[\s\S]*?<button class="list-edit" id="editDreams"[^>]*>Edit<\/button>[\s\S]*?<p class="date">Your private archive<\/p>[\s\S]*?<h1 class="sr-only">History<\/h1>[\s\S]*?<\/header>/);
});
