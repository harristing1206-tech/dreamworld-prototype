...
test('Dreamworld uses one restrained SF Pro typography token system', () => {
  assert.match(html, /--font-ui:-apple-system,BlinkMacSystemFont,"SF Pro Text","Helvetica Neue",sans-serif/);
...
test('core reading and numeric surfaces use the refined typography roles', () => {
  ...
  assert.match(html, /\.date,\.log-kicker\{[^}]*font-size:12px[^}]*font-weight:600[^}]*letter-spacing:\.06em/);
});

test('font weights are normalized to the four deliberate SF Pro roles', () => {
  const weights = [...html.matchAll(/font-weight:(\d+)/g)].map(match => Number(match[1]));
  const unexpected = [...new Set(weights.filter(weight => ![400, 500, 600, 700].includes(weight)))];
