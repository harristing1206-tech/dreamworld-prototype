...
class RuntimeTokenTests(unittest.TestCase):
    def test_gbrain_http_never_prints_bootstrap_token(self):
        ...
        self.assertIn('prompt = "dreamworld readiness probe"', preflight)
        self.assertNotIn('prompt_tokens != 2048', preflight)
        self.assertNotIn('6000', preflight)
        ...
        self.assertNotIn("require-private-encrypted-swap", ollama_unit)
        self.assertIn("OLLAMA_HOST=127.0.0.1:11434", ollama_unit)
        self.assertIn("OLLAMA_NUM_PARALLEL=1", ollama_unit)
        self.assertIn("OLLAMA_MAX_LOADED_MODELS=1", ollama_unit)
        self.assertIn("MemoryHigh=3G", ollama_unit)
        self.assertIn("MemoryMax=4G", ollama_unit)
        self.assertIn("MemorySwapMax=0", ollama_unit)
        self.assertIn("OOMPolicy=stop", ollama_unit)
        self.assertIn("StartLimitIntervalSec=300", ollama_unit)
        self.assertIn("StartLimitBurst=2", ollama_unit)
